...
let groups = {}; // groupId -> group object
...
app.post('/send', (req, res) => {
  const { token, recipient_id, ciphertext_b64, nonce_b64, sender_eph_pub_b64, kem_ciphertext_b64, recipient_kyber_priv_b64, recipient_x25519_priv_b64, ttl_seconds, metadata } = req.body;
  ...
  if (policy === 'pending_approval') {
    ...
    messages[token] = { recipient_id, ciphertext_b64, nonce_b64, sender_eph_pub_b64, kem_ciphertext_b64, expiry: now() + (ttl_seconds || 30), createdAt: now(), state: 'pending', request_id };
    return res.json({ status: 'queued', risk, policy, message: 'Pending approval', request_id });
  } else if (policy === 'require_reauth') {
    messages[token] = { recipient_id, ciphertext_b64, nonce_b64, sender_eph_pub_b64, kem_ciphertext_b64, expiry: now() + (ttl_seconds || 30), createdAt: now(), state: 'require_reauth' };
  ...
  // Store encrypted data with original message and private keys for decryption
  messages[token] = { 
    recipient_id, 
    ciphertext_b64, 
    nonce_b64, 
    sender_eph_pub_b64, 
    kem_ciphertext_b64, 
    recipient_kyber_priv_b64, // Store recipient's private keys for decryption
    recipient_x25519_priv_b64,
    original_message: originalMessage, // Store for demo decryption
    expiry: now() + (ttl_seconds || 30), // 30 seconds default
    createdAt: now(), 
    state: 'stored' 
  };
  return res.json({ status: 'stored', risk, policy, message: `Stored; will expire in ${ttl_seconds || 30}s` });
});
...
app.get('/fetch/:token', (req, res) => {
  ...
  console.log(`✅ Message found, returning encrypted data with private keys`);
  ...
  return res.json({
    ...
    recipient_kyber_priv_b64: rec.recipient_kyber_priv_b64, // Include private keys for decryption
    recipient_x25519_priv_b64: rec.recipient_x25519_priv_b64,
    ttl_remaining,
...
app.post('/groups/create', (req, res) => {
  const { groupId, groupName, creatorId, members, kyber_pub_b64, kyber_priv_b64, x25519_pub_b64, x25519_priv_b64, createdAt, isActive } = req.body;
  
  console.log(`📝 Creating group: ${groupName} (${groupId})`);
...
app.post('/groups/:groupId/add-member', (req, res) => {
  ...
  const { memberId, kyber_pub_b64, kyber_priv_b64, x25519_pub_b64, x25519_priv_b64, addedAt } = req.body;
  ...
  // Store member's keys (in real app, this would be handled differently)
  if (!groups[groupId].memberKeys) {
    groups[groupId].memberKeys = {};
  }
  groups[groupId].memberKeys[memberId] = {
    kyber_pub_b64,
    kyber_priv_b64,
    x25519_pub_b64,
    x25519_priv_b64,
  ...
  res.json({
    success: true,
    message: `Member ${memberId} added to group successfully`
  });
});
 ...
 */
app.post('/groups/:groupId/remove-member', (req, res) => {
  const { groupId } = req.params;
  ...
  // Remove member from group
  groups[groupId].members = groups[groupId].members.filter(id => id !== memberId);
  
  // Remove member's keys
  if (groups[groupId].memberKeys && groups[groupId].memberKeys[memberId]) {
    delete groups[groupId].memberKeys[memberId];
  }
  
  res.json({
    success: true,
    message: `Member ${memberId} removed from group successfully`
  });
});
