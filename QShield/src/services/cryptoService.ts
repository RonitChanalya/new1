 ...
 * Modified to generate real Kyber keypairs locally instead of fetching from server
 */
export async function encryptAndSendHybrid(plainText: string, recipientId: string, opts?: { ttl_seconds?: number, metadata?: any }) {
  ...
  // 2) Generate real Kyber keypair for recipient (demo: generate fresh each time)
  console.log('🔧 Generating real Kyber keypair for recipient:', recipientId);
  const recipientKyberKeypair = await generateKyberKeypair();
  const kyberPub_b64 = recipientKyberKeypair.publicKey_b64;

  // 3) kyber encapsulate (adapter)
  const { kemCiphertextBytes, kemShared } = await kyberEncapsulate(kyberPub_b64);

  // 4) Generate X25519 keypair for recipient (demo: generate fresh each time)
  const recipientX25519Keypair = nacl.box.keyPair();
  const recipX25519Pub = recipientX25519Keypair.publicKey;
  const ecdhShared = new Uint8Array(nacl.box.before(recipX25519Pub, ephPriv)); // 32-byte shared key

  // 5) derive symmetric key using both Kyber and ECDH shared secrets
  ...
  // 7) send to server with recipient's private keys for decryption
  const token = uuidv4();
  const body = {
    token,
    ...
    // Include recipient's private keys for decryption (in real app, these would be stored securely)
    recipient_kyber_priv_b64: recipientKyberKeypair.privateKey_b64,
    recipient_x25519_priv_b64: u8ToB64(recipientX25519Keypair.secretKey),
    original_message: plainText, // Send original message for demo purposes
    ttl_seconds: opts?.ttl_seconds ?? 30, // 30 seconds default
...
export async function fetchAndDecryptHybrid(token: string, recipientPrivateKyber_b64?: string, recipientX25519Priv_b64?: string) {
  ...
  // Use private keys from the message data (sent during encryption)
  let kyberPriv_b64 = recipientPrivateKyber_b64 || data.recipient_kyber_priv_b64;
  let x25519Priv_b64 = recipientX25519Priv_b64 || data.recipient_x25519_priv_b64;
  
  if (!kyberPriv_b64 || !x25519Priv_b64) {
    throw new Error('Recipient private keys not available in message data');
  }
